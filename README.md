...
- N:M 관계를 해소하기 위한(-> 1:N, M:1) 목적으로 인위적으로 만들어진 엔터티
- 다만 교차 엔터티는 FK를 제외한 다른 속성이 추가 되면 교차 엔터티가 아니게 됨
```
   학생   |      |   과목
---------|  N:M | ---------
   학번   |      |   과목코드
...
- 속성도 집합이다.
- 각 속성은 가질 수 있는 값의 범위가 있는데, 이를 도메인이라 한다. 앤터티 내에서 속성에 대한 데이터 타입과 크기, 제약 사항을 지정하는 것이다. 도메인 외의 값을 가질 순 없다.
<br/>
